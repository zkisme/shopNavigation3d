...
THREE.OrbitControls = function ( object, domElement ) {
    ...
    var prevAngle, endAngle, prevScale, endScale;
    ...
    scope.rotateStart = function(event){
        prevAngle = event.rotation;
    }

    scope.rotateMove = function(event){
        endAngle = event.rotation;
        if( endAngle - prevAngle < -180) {
            prevAngle -= 360;
        }else if(endAngle - prevAngle > 180){
            prevAngle += 360;
        }
        var _a = (endAngle - prevAngle) * 0.01;
        rotateLeft2(_a)
        scope.update();
        prevAngle = endAngle;
        scope.object.updateMatrixWorld();
    ...
    scope.scaleStart = function(event){
        prevScale = event.scale;
    }

    scope.scaleMove = function(event){
        endScale = event.scale;
        var _s = endScale - prevScale;
        if(spherical.radius * (scale - _s) > 2000) return;
